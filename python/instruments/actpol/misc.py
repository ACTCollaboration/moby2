...
def get_tod(
    ...
    if use_filebase:
        ...
        filename_ = filebase.get_full_path(filename)
    ...
    if extract in [None, True]:
        extract = 'fb_filt'
    if extract:
        extractor = mce.MCE_data_modes[str(data_mode)][extract]
    else:
        extractor = None
    ...
    if self.enc_flags is None:
        # One might argue this should be np.zeros, not np.ones.
        self.pointing_mask = np.ones(self.nsamps, bool)
    elif repair_pointing:
        max_gap_size = 5
...
def get_hwp_angles(params, tod=None):
    ...
    try:
        ...
        pass
    # Then try Pa* fields.
    pa_num = tod.info.array[-1]
    field_name = 'Hwp_Pa%s_Ang' % pa_num
    angles = tod.get_hk(field_name)
    return angles
